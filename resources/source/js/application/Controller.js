...
class Controller {
   ...
   */
  init() {
    this._isMobileDevice = window.navigator.userAgent
      .toLowerCase()
      .includes("mobi");
    ...
    this._dataLoaderCompleteHandler =
      this._dataLoaderCompleteHandler.bind(this);
    this._deselectAnimationEndHandler =
      this._deselectAnimationEndHandler.bind(this);
    this._removeAnimationEndHandler =
      this._removeAnimationEndHandler.bind(this);
    this._updateContentHandler = this._updateContentHandler.bind(this);
    this._appendContentAnimationEndHandler =
      this._appendContentAnimationEndHandler.bind(this);
    this._bookPageRightAnimationEndHandler =
      this._bookPageRightAnimationEndHandler.bind(this);

    const dataLoader = new Loader(
      C.Assets.CONTENT_MAP_URL,
  ...
  _updateContentHandler(textFieldID) {
    ...
    const selectedTextField = document.getElementById(
      this._selectedTextFieldID
    );
    ...
    if (contentElement.getAttribute(C.HTML.TYPE) !== C.Data.LEAF) {
      ...
      for (const textElement of textElements) {
        if (
          ...
          textElement.classList.contains(C.CSSAnimation.TEXT_FIELD_SELECT)
        ) {
  ...
  _appendContent(ID) {
    ...
    if (selectedTextField) {
      ...
      const parentIsRoot =
        selectedTextField.parentElement.getAttribute(C.HTML.TYPE) ===
        C.Data.ROOT;
      const textFieldHeight = selectedTextField.clientHeight;

      bracketStemWidth = parentIsRoot
        ? 0
        : selectedTextField.parentElement.clientWidth -
          selectedTextField.clientWidth;
      bracketStemHeight =
        textFieldHeight *
          Array.from(selectedTextField.parentElement.children).indexOf(
            selectedTextField
          ) +
        textFieldHeight / 2;
    }
    ...
    contentNode.element.addEventListener(
      C.Event.ANIMATION_END,
      this._appendContentAnimationEndHandler
    );
