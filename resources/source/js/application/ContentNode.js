...
class ContentNode extends Dispatcher {
  ...
  _init() {
    if (
      this._contentParentNode instanceof Node &&
      this._contentParentNode.nodeType === Node.ELEMENT_NODE &&
      this._contentParentNode.hasChildNodes()
    ) {
      ...
      this._appendChildNodes(
        C.Data.ATTRIBUTE_URL,
        this._createPicture.bind(this),
      );
      this._appendChildNodes(
        C.Data.ATTRIBUTE_LABEL,
        this._createTextField.bind(this),
      );
      ...
      if (nodeType === C.Data.ROOT) {
        ...
        this._bracket = new Bracket(
          this._bracketStemWidth,
          this._bracketStemHeight,
        );
        ...
        if (nodeType === C.Data.LEAF) {
          ...
          if (this._picture) {
            if (this._picture.isZoomable) {
              ...
              footnoteNode.setAttribute(
                C.Data.ATTRIBUTE_LABEL,
                C.Loupe.ZOOM_INSTRUCTION,
              );
              footnoteNode.setAttribute(
                C.Data.ATTRIBUTE_FOOTNOTE,
                C.Data.ATTRIBUTE_FOOTNOTE,
              );

              this._element.appendChild(
                this._createTextField(footnoteNode).element,
              );
            }

            this._element.insertBefore(
              this._bracket.element,
              this._picture.element,
            );
          }
        } else {
  ...
  _createPicture(elementNode) {
    ...
    const isZoomable =
      elementNode.getAttribute(C.Data.ATTRIBUTE_ZOOMABLE) &&
  ...
  _createTextField(elementNode) {
    ...
    this._textFields.unshift(
      new TextField(label, id, href, download, footnote),
    );

    if (elementNode.hasChildNodes()) {
      this._textFields[0].addListener(C.Event.CLICK, (id) =>
        super.dispatch(C.Event.UPDATE_CONTENT, id),
      );
    }
  ...
  _updateLeafSize(offsetWidth = 0, offsetHeight = 0) {
    ...
    if (
      ...
      viewPort.width === this._previousViewPort.width &&
      viewPort.height === this._previousViewPort.height
    ) {
      return;
    }
    ...
    const margin = parseFloat(
      window
        .getComputedStyle(C.HTMLElement.MAIN, null)
        .getPropertyValue(C.CSS.MARGIN_TOP),
    );
    const maxWidth =
      viewPort.width - elementPosition.left - margin + offsetWidth;
    const picture = this._picture;
    ...
    if (picture) {
      if (!this._pictureMinWidth) {
        this._pictureMinWidth = parseFloat(
          window
            .getComputedStyle(picture.element, null)
            .getPropertyValue(C.CSS.MIN_WIDTH),
        );
      }
