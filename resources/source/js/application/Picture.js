...
class Picture {
    ...
    _init() {
        ...
        this._outline = new Outline(this._nativeWidth, this._nativeHeight, C.Color.GRAY_CHANNEL);
        ...
        this._contentLoader.addListener(Loader.Event.PROGRESS, this._contentLoaderProgressHandler.bind(this));
        this._contentLoader.addListener(Loader.Event.COMPLETE, this._contentLoaderCompleteHandler.bind(this));
    }
    ...
    _contentLoaderCompleteHandler(data) {
        ...
        this._element.addEventListener(C.Event.ANIMATION_END, this._fadeOutAnimationHandler);
    ...
    _mouseMoveHandler(event) {
        ...
        this._loupe.updatePosition(this._mouseX, this._mouseY, this._width, this._height);
    }
    ...
    _mouseWheelHandler(event) {
        ...
        const zoomDirection = (event.deltaY < 0) ? Loupe.ZoomDirection.ZOOM_IN : Loupe.ZoomDirection.ZOOM_OUT;
        const accelerate = event.shiftKey;

        this._loupe.updateZoom(zoomDirection, this._mouseX, this._mouseY, this._width, this._height, accelerate);
    }
    ...
    _keyDownHandler(event) {
        ...
        */
        ...
        this._mouseWheelHandler({deltaY: zoomDirection * -1, shiftKey: event.shiftKey});
    }
    ...
    _fadeOutAnimationHandler(event) {
        ...
        element.removeEventListener(C.Event.ANIMATION_END, this._fadeOutAnimationHandler);
        ...
        element.addEventListener(C.Event.ANIMATION_END, this._fadeInAnimationHandler);
    ...
    _fadeInAnimationHandler(event) {
        ...
        const element = event.target;
        element.removeEventListener(C.Event.ANIMATION_END, this._fadeInAnimationHandler);
    ...
    resize(maxWidth, maxHeight, minWidth, offsetX, offsetY) {
        ...
        if (maxWidth < nativeWidth || maxHeight < nativeHeight) {
            ...
            ratio = Math.max(minWidth / nativeWidth, Math.min(ratioWidth, ratioHeight));
        ...
        if (!this._isLoaded) {
            ...
            this._imageLoadProgressHeight = this._height - this._book.element.offsetHeight;
        }
    }
    ...
    dispose() {
        ...
        this._element.removeEventListener(C.Event.ANIMATION_END, this._fadeOutAnimationHandler);
        this._element.removeEventListener(C.Event.ANIMATION_END, this._fadeInAnimationHandler);
        this._element = null;
        ...
        if (this._content) {
            ...
            this._content.removeEventListener(C.Event.CONTEXT_MENU, this._contextMenuHandler);
            this._content.removeEventListener(C.Event.DRAG_START, this._dragStartHandler);
            
            if (this._isZoomable) {

                this._content.removeEventListener(C.Event.MOUSE_ENTER, this._mouseEnterHandler);
                this._content.removeEventListener(C.Event.MOUSE_MOVE, this._mouseMoveHandler);
                this._content.removeEventListener(C.Event.MOUSE_LEAVE, this._mouseLeaveHandler);
                this._content.removeEventListener(C.Event.MOUSE_WHEEL, this._mouseWheelHandler);
