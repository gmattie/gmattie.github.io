...
class Loupe {
    ...
    _init() {
        ...
        this._skinLoader.addListener(Loader.Event.COMPLETE, this._skinLoaderCompleteHandler.bind(this));
    ...
    _restoreZoom() {
        ...
        this._zoomLevel = (cachedZoomLevel) ? Math.max(this._minimumZoom, cachedZoomLevel) : Math.max(this._minimumZoom, C.Loupe.ZOOM_DEFAULT);
        this.updateZoom();
     ...
     * 
     */
    updateZoom(direction = null, x = null, y = null, width = null, height = null, accelerate = false) {

        if (direction) {

            const zoomIn = (accelerate) ? C.Loupe.ZOOM_STEP_ACCELERATED : C.Loupe.ZOOM_STEP;
            const zoomOut = -zoomIn;
            const zoomStep = (direction === Loupe.ZoomDirection.ZOOM_IN) ? zoomIn : zoomOut;
            
            this._zoomLevel = Math.min(Math.max(this._minimumZoom, this._zoomLevel += zoomStep), C.Loupe.ZOOM_MAX);
    ...
    updatePosition(x, y, width, height) {
        ...
        const contentX = -(this._content.clientWidth * this._zoomLevel * fractionX) / this._zoomLevel - this._maskOffset.tx / this._zoomLevel + this._skinHalfSize / this._zoomLevel;
        const contentY = -(this._content.clientHeight * this._zoomLevel * fractionY) / this._zoomLevel - this._maskOffset.ty / this._zoomLevel + this._skinHalfSize / this._zoomLevel;

        this._element.style.transform = `${C.CSS.TRANSLATE}(${x - this._skinHalfSize}${C.CSS.PX}, ${y - this._skinHalfSize}${C.CSS.PX})`;
