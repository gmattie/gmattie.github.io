...
class Book {
  ...
  _draw() {
    ...
    const leftPageContext = leftPageCanvas.getContext(
      C.HTML.CANVAS_RENDERING_CONTEXT_2D,
    );
    leftPageContext.fillStyle = this._color;
    leftPageContext.transform(1, C.Book.SKEW, 0, 1, 0, 0);
    leftPageContext.fillRect(
      0,
      0,
      C.Book.WIDTH * this._scale,
      (C.Book.HEIGHT / 2) * this._scale,
    );
    ...
    const rightPageContext = rightPageCanvas.getContext(
      C.HTML.CANVAS_RENDERING_CONTEXT_2D,
    );
    rightPageContext.fillStyle = this._color;
    rightPageContext.transform(
      1,
      -C.Book.SKEW,
      0,
      1,
    ...
    rightPageContext.fillRect(
      C.Book.GAP * this._scale,
      0,
      C.Book.WIDTH * this._scale,
      (C.Book.HEIGHT / 2) * this._scale,
    );
